...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 货架管理 <span class="c-gray en">&gt;</span> 补货员列表 <a id="refresh" class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container">
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"><a href="javascript:;" onclick="datadel(0)" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6dc;</i> 批量上线</a> <a href="javascript:;" onclick="datadel(1)" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6de;</i> 批量下线</a> <a class="btn btn-primary radius" data-title="添加补货员" data-href="article-add.html" onclick="add_news()" href="javascript:;"><i class="Hui-iconfont">&#xe600;</i> 添加补货员</a></div>
	<div class="mt-20">
		<table class="table table-border table-bordered table-bg table-hover table-sort table-responsive">
            ...
            $sql = $mysqli->query("select * from replenishman_info order by gmt_modified desc");
            $datarow = mysqli_num_rows($sql); //长度
            //循环遍历出数据表中的数据
            for($i=0;$i<$datarow;$i++){
                $sql_arr = mysqli_fetch_assoc($sql);
                $id = $sql_arr['id'];
                $name = $sql_arr['replenishman_name'];
                $phone = $sql_arr['replenishman_phone'];
                $idcard = $sql_arr['replenishman_idcard'];
                $address = $sql_arr['replenishman_address'];
                ...
                echo "<tr class='text-c'>
                                ...
                                <td class='biz_id' hidden='hidden'>$id</td>
...
$(".table-sort tbody tr").dblclick(function (){
    ...
    var id =$(this).children('.biz_id').html()
    var url ='replenish-edit.php?id='+id;
...
function edit(ele) {
    ...
    var id = $(ele).siblings('.biz_id').html();
    var url ='replenish-edit.php?id='+id;
    var index = layer.open({
        type: 2,
        title: '新闻编辑',
...
function datadel(type) {
    ...
    if (vals.length < 1) {
        ...
        var ids = vals.join(',');
        commonOnline(text + vals.length + '个补货员吗？',ids,type);
    }
}
...
function add_news() {
    var index = layer.open({
        type: 2,
        title: '添加商品',
        content: 'replenish-edit.php?id=0'
    });
    layer.full(index);
}
