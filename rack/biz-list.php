...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 货架管理 <span class="c-gray en">&gt;</span> 业务员列表 <a id="refresh" class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container">
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"><a href="javascript:;" onclick="datadel(0)" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6dc;</i> 批量上线</a> <a href="javascript:;" onclick="datadel(1)" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6de;</i> 批量下线</a> <a class="btn btn-primary radius" data-title="添加业务员" data-href="article-add.html" onclick="add_news()" href="javascript:;"><i class="Hui-iconfont">&#xe600;</i> 添加业务员</a></div>
	<div class="mt-20">
		<table class="table table-border table-bordered table-bg table-hover table-sort table-responsive">
			...
			<tbody>
                <?php
                    ...
                    $sql = $mysqli->query("select * from biz_man_info order by gmt_modified desc");
                    $datarow = mysqli_num_rows($sql); //长度
                    //循环遍历出数据表中的数据
                    for($i=0;$i<$datarow;$i++){
                        $sql_arr = mysqli_fetch_assoc($sql);
                        $id = $sql_arr['id'];
                        $name = $sql_arr['biz_man_name'];
                        $phone = $sql_arr['biz_man_phone'];
                        $idcard = $sql_arr['biz_man_idcard'];
                        $address = $sql_arr['biz_man_address'];
...
function datadel(type) {
    ...
    if (vals.length < 1) {
        ...
        var ids = vals.join(',');
        commonOnline(text + vals.length + '个业务员吗？',ids,type);
    }
}
