		...
		<table class="table table-border table-bordered table-bg table-hover table-sort table-responsive">
			...
			<tbody>
                <?php
                    ...
                    $sql = $mysqli->query("select * from goods_info order by gmt_modified desc");
                    $datarow = mysqli_num_rows($sql); //长度
                    //循环遍历出数据表中的数据
                    for($i=0;$i<$datarow;$i++){
                        $sql_arr = mysqli_fetch_assoc($sql);
                        $name = $sql_arr['goods_name'];
                        $stateId = $sql_arr['goods_tag'];
                        $tagIdArray = explode(',',$stateId);
                        $tagName = '';
                        $length = count($tagIdArray);
                        for($j=0;$j<$length;$j++){
                            $sql1 = $mysqli->query("select state_name from state_info where ename='goods_tag' and state_id='$tagIdArray[$j]'");
                            $sql_arr1 = mysqli_fetch_assoc($sql1);
                            $tagName .= $sql_arr1['state_name'].'，';
                        }
                        $tagName = mb_substr($tagName,0,mb_strlen($tagName)-1);
                        $url = $sql_arr['goods_uri'];
                        ...
                        $sql2 = $mysqli->query("select state_name from state_info where ename='goods_category' and state_id='$category'");
                        $sql_arr2 = mysqli_fetch_assoc($sql2);
                        $categoryName = $sql_arr2['state_name'];
                        $time = $sql_arr['gmt_modified'];
                        $special = $sql_arr['is_special_price']==0?'是':'否';
                        $limit = $sql_arr['is_limited']==0?'是':'否';
                        ...
                        echo "<tr class='text-c'>
                                ...
                                <td><img width='210' class='picture-thumb' src='$url'></td>
                                <td>$oldPrice</td>
                                <td>$nowPrice</td>
                                <td>$discount</td>
                                <td>$categoryName</td>
...
function datadel(type) {
    ...
    if (vals.length < 1) {
        ...
        var ids = vals.join(',');
        commonOnline(text + vals.length + '个商品吗？',ids,type);
    }
}
